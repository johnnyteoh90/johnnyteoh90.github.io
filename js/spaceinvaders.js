...
PlayState.prototype.draw = function (game, dt, ctx) {
  ...
  // new image instance for spaceship image
  var shipImg = new Image();

  // setting the src of the instance
  shipImg.src = "../Spaceship.png";

  // drawing the instance on the canvas
  ctx.drawImage(
    shipImg,
    this.ship.x - this.ship.width / 2,
    this.ship.y - this.ship.height / 2,
    this.ship.width,
    this.ship.height + 10 // the image was getting stretched so I added 10 to the value of the height
  );
  ...
  for (var i = 0; i < this.invaders.length; i++) {
    var invader = this.invaders[i];
    // creating a new img instance for adding to canvas
    var invaderImg = new Image();
    // setting the source of the image
    invaderImg.src = "../newship.png";
    // ctx.fillRect(invader.x - invader.width/2, invader.y - invader.height/2, invader.width, invader.height);
    // using the drawImage method to draw the image on the canvas with the specified params
    ctx.drawImage(
      invaderImg,
